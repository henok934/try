...
from models.buschange import Buschange
...
def ticket():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('database.db')
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM tickets WHERE date=? AND depcity=? AND firstname=? AND lastname=? AND side_no=? AND descity=? AND plate_no=?", (date, depcity, firstname, lastname, side_no, descity, plate_no))
        Same_name = cursor.fetchone()
        conn.close()
        if len(phone) != 10:
            return render_template('ticket.html', error="Phone number must be 10 digits!")
        elif not no_seat.isdigit():
            return render_template('ticket.html', error="The Bus is full!.")
        elif Same_name:
            return render_template('ticket.html', error="This man is already booked!")
        else:
            info = {
                "firstname": firstname,
                "lastname": lastname,
...
def route():
    ...
    if request.method == 'POST':
        ...
        if same_date:
            ...
            new_account.save()
            if depcity == "Addisabeba":
                date_obj = datetime.strptime(date, "%Y-%m-%d")
                next_date_obj = date_obj + timedelta(days=1)
                next_date = next_date_obj.strftime("%Y-%m-%d")

                # Modify the info dictionary with updated values
                info["depcity"] = descity
                info["descity"] = depcity
                info["date"] = next_date
                new_account = Route(**info)
                new_account.save()
...
def registor():
    if request.method == 'POST':
        ...
        username = data.get('username')
        if username in usernames:
            return render_template('registor.html',error="username is already exist")
        elif email in emails:
...
@views.route('/worker', methods=['GET', 'POST'], strict_slashes=False)
@views.route('/')
def worker():
    data = []
    buses = storage.all(Bus)
    for bus in buses.values():
        data.append(bus.to_dict())
    if request.method == 'POST':
        users = storage.all(Worker).values()
        usernames = [user.username for user in users]
        phones = [user.phone for user in users]
        data = request.form
        print(data)
        ...
        username = data.get('username')
        """
        cursor.execute("SELECT * FROM workers WHERE plate_no=? AND side_no ", (plate_no, side_no))
        worker = cursor.fetchone()

        """
        if username in usernames:
            return render_template('worker.html',error="username is already exist")
        elif phone in phones:
            flash("Phone number already exists", category='error')
        else:
            info = {"fname": fname, "lname": lname,
            ...
            for bus in buses.values():
                data.append(bus.to_dict())
        return render_template('worker.html',data=data, success="Worker Registor Successfully!")
    return render_template("worker.html", data=data, user=current_user)
...
@views.route('/ticketinfo', methods=['GET', 'POST'])
def ticketinfo():
    raw_cities = storage.all(City)
    des = [city.to_dict() for city in raw_cities.values()]
    if request.method == 'POST':
        ...
        for bus in buses.values():
            if bus.depcity == depcity and bus.descity == descity and bus.date == date:
                routes.append(bus.to_dict())
        if routes:
            return render_template('rootee.html', routes=routes, success="Route info Goto Ticket page!! ")
        else:
            return render_template('cheeckrouteee.html', des=des,error='There is no Travel in this city and date!')
    return render_template('cheeckrouteee.html', des=des)


@views.route('/Selectbus', methods=['GET', 'POST'])
def Selectbus():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('database.db')
        c = conn.cursor()
        c.execute("SELECT * FROM tickets WHERE plate_no = ? AND side_no = ? AND date = ?", (plate_no, side_no, date))
        routes = c.fetchall()
        conn.close()
        if routes:
            return render_template('ticketoch.html', routes=routes)
        else:
            return render_template('rootee.html', error="No ticket booked")

    return render_template('rootee.html')

@views.route('/Showtickets', methods=['GET', 'POST'])
def Showtickets():
    if request.method == 'POST':
        plate_no = request.form['plate_no']
        side_no = request.form['side_no']
        ...
        conn = sqlite3.connect('database.db')
        c = conn.cursor()
        c.execute("SELECT * FROM tickets WHERE plate_no = ? AND side_no = ? AND date = ? AND depcity = ? AND descity = ?", (plate_no, side_no, date, depcity, descity))
        routes = c.fetchall()
        conn.close()
        if routes:
            return render_template('ticketoche.html', routes=routes)
        else:
            return render_template('rooteee.html', error="No ticket booked")
    return render_template('rootee.html')
...
@views.route('/workerdelete', methods=['GET', 'POST'])
def workerdelete():
    if request.method == 'POST':
        plate_no = request.form['plate_no']
        side_no = request.form['side_no']
        ...
        conn = sqlite3.connect('database.db')
        cursor = conn.cursor()
        cursor.execute('DELETE FROM workers WHERE fname = ? AND lname=?  AND plate_no=? AND side_no=?', (fname, lname,  plate_no, side_no))
        rows_deleted = cursor.rowcount

        conn.commit()
        conn.close()
        workers = []
        worke = storage.all(Worker)
        for city in worke.values():
            workers.append(city.to_dict())
        if rows_deleted > 0:
            return render_template('workerdelete.html', workers=workers, success="Route Deleted Successfully!")
        else:
            return render_template('workerdelete.html', workers=workers)
    workers = []
    cite = storage.all(Worker)
    for city in cite.values():
        workers.append(city.to_dict())
    if routes:
        return render_template('workerdelete.html', workers=workers, success="Route Deleted Successfully!")
...
@views.route('/changebus', methods=['POST', 'GET'])
def changebus():
    if request.method == 'POST':
        plate_no = request.form['plate_no']
        side_no = request.form['side_no']
        ...
        descity = request.form['descity']
        date = request.form['date']
        new_side_no = request.form['new_side_no']
        new_plate_no = request.form['new_plate_no']
        conn = sqlite3.connect('database.db')
        cursor = conn.cursor()
        cursor.execute('SELECT COUNT(no_seat) FROM tickets WHERE depcity = ? AND descity = ? AND date = ? AND plate_no = ?', (depcity, descity, date, plate_no))
        seatnumber = cursor.fetchone()
        ...
        conn = sqlite3.connect('database.db')
        c = conn.cursor()
        c.execute("SELECT no_seats FROM buses WHERE plate_no = ? AND sideno = ?", (new_plate_no, new_side_no,))
        no_seat = c.fetchone()
        conn.close()
        ...
        print(wenberkutr)
        print(seats)
        if wenberkutr > str(seats):
            routes = []
            buse = storage.all(Route)
            for bus in buse.values():
                routes.append(bus.to_dict())
            data = []
            buses = storage.all(Bus)
            for bus in buses.values():
                data.append(bus.to_dict())
            return render_template('buschange.html', data=data, routes=routes, error="New inserted bus does not have enough seats!")
        else:
            conn = sqlite3.connect('database.db')
            cursor = conn.cursor()
            cursor.execute(
                    'UPDATE routes SET side_no=?, plate_no=? WHERE plate_no=? AND depcity=? AND descity=? AND side_no=? AND date=?',
                    (new_side_no, new_plate_no, plate_no, depcity, descity, side_no, date))
            conn.commit()
            cursor.execute(
                    'UPDATE tickets SET side_no=?, plate_no=? WHERE plate_no=? AND depcity=? AND descity=? AND side_no=? AND date=?',
                    (new_side_no, new_plate_no, plate_no, depcity, descity, side_no, date))
            conn.commit()
            conn.close()
            info = {
        ...
        for bus in buses.values():
            data.append(bus.to_dict())
        return render_template('buschange.html', data=data, routes=routes, success="Bus changed successfully!")
    data = []
    buses = storage.all(Bus)
    for bus in buses.values():
        data.append(bus.to_dict())
    ...
    if routes:
        return render_template('buschange.html', data=data, routes=routes)
    else:
        return render_template('error.html', error="There are no buses available for change!")
"""
@views.route('/changebus', methods=['POST', 'GET'])
def changebus():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('database.db')
        cursor = conn.cursor()
        cursor.execute('SELECT COUNT(no_seat) FROM tickets WHERE depcity = ? AND descity = ? AND date = ? AND side_no = ? AND plate_no = ?', (depcity, descity, date, plate_no, side_no))
        seatnumber = cursor.fetchone()
        conn.close()
        conn = sqlite3.connect('database.db')
        c = conn.cursor()
        ...
        if no_seat:
            seats = int(no_seat[0][0])  # Convert seats to an integer
        if numcount > seats:
            return render_template("buschange.html", error="New inserted bus does not have enough seats!")
        else:
            conn = sqlite3.connect('database.db')
            cursor = conn.cursor()
            cursor.execute(
            ...
            conn.commit()
            conn.close()
            info = {"new_side_no": new_side_no, "new_plate_no": new_plate_no,
                    "side_no": side_no, "depcity": depcity, "descity": descity, "date": date, "plate_no": plate_no
                    }
            new_account = Buschange(**info)
            new_account.save()
            routes = []
...
def changebus():
    if request.method == 'POST':
        ...
        if seats < numcount:
            return render_template("buschange.html", error="New inserted bus is not enough seat!")
        else:
            cursor.execute(
            'UPDATE routes SET side_no=?, plate_no=? WHERE plate_no=? AND depcity=? AND descity=? AND side_no=? AND date=?',
            (new_side_no, new_plate_no, plate_no, depcity, descity, side_no, date))
        ...
        if routes:
            ...
            return render_template('error.html', error="There are no buses for change!")
"""
@views.route('/changebuses')
def changebuses():
    buschange = []
    buse = storage.all(Buschange)
    for bus in buse.values():
        buschange.append(bus.to_dict())
    if buschange:
        return render_template('busschange.html', buschange=buschange)
    else:
        return render_template('error.html', error="NO change buses!.")
@views.route('/profile')
def profile():
