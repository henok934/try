...
def login():
    if request.method == 'POST':
        ...
        if role == 'user':
            ...
            if admin:
                return render_template('ad.html', admin=admin)
            else:
                return render_template('login.html', error='Invalid username, password, or role')
        elif role == 'worker':
            c.execute("SELECT * FROM workers WHERE username = ? AND password = ?", (username, hashed_password))
            worker = c.fetchone()

        if worker:

            c.execute("SELECT side_no FROM workers WHERE username = ? AND password = ?", (username, hashed_password))
            work = c.fetchone()
            if work:
                side_no = work[0]
                print(side_no)
                c.execute("SELECT date, depcity, descity, side_no, plate_no FROM routes WHERE side_no = ?", (side_no,))
                rows = c.fetchall()
                routes = rows
                conn.close()
                return render_template('rooteee.html', routes=routes)
        else:
            conn.close()
            return render_template('login.html', error='Invalid username, password, or role')
    else:
